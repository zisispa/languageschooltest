...
@section('content')
    ...
    @if ($general_announcements->count() > 0)
        <div id="accordion">
            @foreach ($general_announcements as $announcement)
                <div class="card mb-3">
                    <div class="card-header bg-gradient-primary p-0" id="heading{{ $announcement->id }}">
    ...
    @if (count($group_announcements) > 0)
        <div id="accordion">
            @foreach ($group_announcements as $group_announcement)
                @foreach ($group_announcement as $announcement)
                    <div class="card mb-3">
                        <div class="card-header bg-gradient-primary p-0" id="heading_group_{{ $announcement->id }}">
                            <a class="btn collapsed" data-toggle="collapse"
                                data-target="#collapse_group_{{ $announcement->id }}" aria-expanded="false"
                        ...
                        <div id="collapse_group_{{ $announcement->id }}" class="collapse"
                            aria-labelledby="heading_group_{{ $announcement->id }}" data-parent="#accordion">
                            <div class="card-body mx-2">
                                <div class="row d-flex justify-content-between align-items-center">
                                    <p class="font-weight-bold">Υποβλήθηκε από: {{ $announcement->teacher->user->name }}</p>
                                    <p>{{ $announcement->created_at->diffForHumans() }}</p>
